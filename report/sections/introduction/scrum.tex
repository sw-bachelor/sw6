...
\\
\\
Then the user stories are distributed among the groups as fair as possible such that each groups has one user story.
Each group then tries to approximate the time required to solve the user story and whether it should be split into smaller tasks.
...
The start of the meeting takes the same form as the start of the Sprint Planning described in \ref{subsec:SoS-sprint-planning}
\\
\\
Afterwards the groups then choose a user story from the highest prioritized user stories in a first comes first served manner.
