...
Unlike the previous sprints where each group got to decide what they wanted to work on, we made an assessment on which user stories needed to be finished in order to have a minimal viable product, and assigned each story to a group based on their previous experiences, such that they would mostly work on expanding their own previously written code.
...
This meant that we did not have any user stories to implement this semester, but rather focused on figuring out what information we needed to pass on, write it down and figure out how to make it easily accessible for next year.

Meanwhile, we had a focus on finishing the last prototypes, so that all user stories had prototypes available for next year, to minimize the amount of work needed by the next PO group.