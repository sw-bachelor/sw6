...
First of all, during the readthrough of the reports from previous years, 
it was discovered that the PO group of 2018 had left us a suggestion for content in the first sprint:

\subsection*{Suggested sprint 1}
The first sprint that the previous PO group suggests includes two user stories. 
The first user story presents the need for a guardian to be able to mark multiple activities and perform actions on this activities together. 
The second one says that the a user must be able to change the way that an activity is marked as being complete. This could for example be represented by a checkmark, by hiding the activity or by moving the activity a bit to the right on the schema for the day.
These user stories are suggested for the first sprint because it should be easy for the developers during the start of the project, 
because they aren't familiar with the codebase yet.

\subsection*{Suggested sprint 2}
This sprint suggests two user stories where the first one is that a user should be able to time activities with a timer. This is needed so the citizens know how long time their activities take. The guardian should be able to add this timer with a specific time and connect to the activity. The citizen should then be the one that starts the timer. 

The second user story concerns a feature for guardians that allows them to choose between a set of visual representations for the timers that they add for the activites. This is needed because the citizens have different preferences when it comes to representation of the time.

Again, this suggested sprint is meant to be quite light in workload to allow the developers to get familiar with the codebase, and to ensure that all groups can finish their tasks for the sprint.
