...
For the release, we started by looking at which functionalities were implemented throughout the sprint and creating an overview of these.
...
We created a pdf that explained what functionality we wanted to see in the release based on the known statuses of the user stories.
We then started to look at which problems existed in the application.
These were the initial problems that we became aware of:
\begin{itemize}
    \item Choose citizen needs to be connected to the week plan selector
...
\noindent We created user stories for these issues and delegated the user stories to the groups that had created the screens they were present on. 
The release branch was merged in to the master and the develop branch when all of the groups had approved their area of responsibility and we had confirmed that they worked as expected. 
Finally a release party was held where all the groups could meet and we presented the current state of the application.
